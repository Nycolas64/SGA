...
const actionsBox = modal.querySelector('.actions-box');

let statusNotice = document.getElementById('detalheStatusAviso');
if (!statusNotice) {
    statusNotice = document.createElement('div');
    statusNotice.id = 'detalheStatusAviso';
    statusNotice.style.padding = '10px 15px';
    statusNotice.style.borderRadius = '6px';
    statusNotice.style.fontWeight = 'bold';
    statusNotice.style.marginBottom = '15px';
    statusNotice.style.textAlign = 'center';
    const infoDetalhe = modal.querySelector('.info-detalhe');
    infoDetalhe.parentNode.insertBefore(statusNotice, actionsBox);
}

const btnAprovar = actionsBox.querySelector('.btn-confirm');
const btnRejeitar = actionsBox.querySelector('.btn-delete-modal');

if (solicitacao.status === 'PENDENTE') {
    if (btnAprovar) btnAprovar.style.display = 'inline-block';
    if (btnRejeitar) btnRejeitar.style.display = 'inline-block';
    ...
    if (solicitacao.status === 'APROVADA') {
        statusNotice.style.backgroundColor = '#d4edda';
        statusNotice.style.color = '#155724';
        statusNotice.innerText = 'Esta solicitação já foi aprovada e não pode mais ser alterada.';
    } else if (solicitacao.status === 'REJEITADA') {
        statusNotice.style.backgroundColor = '#f8d7da';
        statusNotice.style.color = '#721c24';
        statusNotice.innerText = 'Esta solicitação foi rejeitada e não pode mais ser alterada.';
    } else {
        statusNotice.style.backgroundColor = '#e2e3e5';
        statusNotice.style.color = '#383d41';
        statusNotice.innerText = `Esta solicitação está finalizada e não pode mais ser alterada.`;
    }
}
